...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> SpalartAllmaras<BasicTurbulenceModel>::DnuTildaEff1(volScalarField nuTilda1,surfaceScalarField phi1)
{
    // Local references
    const alphaField& alpha = this->alpha_;
    const rhoField& rho = this->rho_;
    const surfaceScalarField alphaRhoPhi = phi1;//this->alphaRhoPhi_;
    nuTilda_ = nuTilda1;
    ...
    nuTilda_.correctBoundaryConditions();
  //  correctNut(fv1);
    //Info<<nuTildaEqn.ref()<<endl;
    return nuTildaEqn;
}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> SpalartAllmaras<BasicTurbulenceModel>::DnuTildaEff2(volScalarField nuTilda1,volScalarField nu1,surfaceScalarField phi1)
{
    // Local references
    const alphaField& alpha = this->alpha_;
    const rhoField& rho = this->rho_;
    ...
    (
        fvm::ddt(alpha, rho, nuTilda_)
      + fvm::div(alphaRhoPhi, nuTilda_)
      - fvm::laplacian(alpha*rho*(nuTilda_ + nu1)/sigmaNut_, nuTilda_)
      - Cb2_/sigmaNut_*alpha*rho*magSqr(fvc::grad(nuTilda_))
     ==
        Cb1_*alpha*rho*Stilda*nuTilda_
      - fvm::Sp(Cw1_*alpha*rho*fw(Stilda)*nuTilda_/sqr(y_), nuTilda_)
